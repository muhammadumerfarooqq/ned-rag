...
**Status:** 🚧 In progress — Phase 5 complete (REST API)

## Pipeline
5. **API** — `src/api.py` wraps the RAG pipeline in a FastAPI service: `POST /query` returns a grounded answer as JSON, `GET /health` for status. Input validation via Pydantic; errors handled without leaking internals.